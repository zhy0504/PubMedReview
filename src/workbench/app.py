...
def create_app(root=None, data_dir=None, command=None):
    ...
    def bootstrap():
        ...
        import os
        settings.update({key: value for key, value in os.environ.items() if key.endswith(('_API_KEY', '_MODEL')) or key == 'DEFAULT_AI_SERVICE'})
        service = settings.get('DEFAULT_AI_SERVICE') or 'openai'
        ...
        return jsonify(stats=store.stats(), preferences=store.preferences(), active_id=manager.active_id,
            configuration={'service': service, 'key_configured': bool(settings.get(service.upper() + '_API_KEY')),
                'model': settings.get(service.upper() + '_MODEL') or system_config.PREFERRED_MODEL,
