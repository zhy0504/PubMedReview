...
from ai_providers import PROVIDER_PROFILES, get_provider
from openai_protocol import is_deepseek_url, is_model_compatible, is_zhipu_url
...
def apply_model_options(system, options):
    for component, model_key, reasoning_key in ((system.intent_analyzer, 'intent_model', 'intent_reasoning'), (system.outline_generator, 'outline_model', 'outline_reasoning'), (system.review_generator, 'review_model', 'review_reasoning')):
        ...
        adapter_config = getattr(getattr(component, 'adapter', None), 'config', None)
        base_url = getattr(adapter_config, 'base_url', '')
        configured_model = getattr(adapter_config, 'default_model', '')
        provider = getattr(adapter_config, 'name', None)
        if not model or not is_model_compatible(base_url, model, provider):
            if is_model_compatible(base_url, configured_model, provider):
        ...
        if hasattr(component, 'adapter') and component.adapter:
            api_type = options.get('ai_protocol', 'openai')
            profile = get_provider(provider)
            if profile and profile.api_type in {'gemini', 'anthropic'}:
                api_type = profile.api_type
            component.adapter.config.api_type = api_type
...
async def execute(options, channel):
    configured_service = (options.get('ai_provider') or os.environ.get('DEFAULT_AI_SERVICE', 'openai')).strip().lower()
    profile = get_provider(configured_service)
    service_name = configured_service if profile else 'openai'
    if profile and profile.requires_api_key and not os.environ.get(f'{profile.env_prefix}_API_KEY'):
        # Preserve the historical automatic fallback when the selected service
        # is not configured, while still honoring local keyless providers.
        for candidate, candidate_profile in PROVIDER_PROFILES.items():
            if not candidate_profile.requires_api_key:
                continue
            candidate_profile = get_provider(candidate)
            if candidate_profile and os.environ.get(f'{candidate_profile.env_prefix}_API_KEY'):
                service_name = candidate
                profile = candidate_profile
                break
    if not profile:
        profile = get_provider('openai')
    os.environ['DEFAULT_AI_SERVICE'] = service_name
    api_type = options.get('ai_protocol', 'openai')
    if profile and profile.api_type in {'gemini', 'anthropic'}:
        api_type = profile.api_type
    os.environ[f'{profile.env_prefix}_API_TYPE'] = api_type
    if options['model']:
        os.environ[f'{profile.env_prefix}_MODEL'] = options['model']
