...
const state = {job: null, activeId: null, after: 0, logs: '', articleOffset: 0, articleTotal: 0, historyOffset: 0, historyTotal: 0, polling: false, reviewId: null, historyRequest: 0, articleRequest: 0};
let toastTimer;
...
function fillOptions(options) {
  ...
  byId('setting-protocol').value = options.ai_protocol || 'openai';
...
async function bootstrap(initial = false) {
  ...
  if (initial) {
    ...
    byId('setting-api-key').placeholder = config.api_key_configured ? '已配置；留空保留' : '请输入 AI API Key';
  ...
  if (initial) {
    fillOptions(data.preferences);
    if (state.activeId) await selectJob(state.activeId);
  }
}
...
byId('save-preferences').addEventListener('click', async () => {
  try { const options = readOptions(); await api('/api/preferences', {method: 'PUT', body: JSON.stringify(options)}); await api('/api/configuration', {method: 'PUT', body: JSON.stringify({base_url: byId('setting-base-url').value, api_key: byId('setting-api-key').value, pubmed_api_key: byId('setting-pubmed-key').value})}); notify('默认运行选项和配置已保存。'); }
  catch (error) { notify(error.message); }
});
byId('fetch-models').addEventListener('click', async () => {
  byId('model-status').textContent = '获取中…';
  try { const data = await api('/api/models', {method: 'POST', body: '{}'}); byId('model-list').replaceChildren(...data.models.map((id) => { const option = document.createElement('option'); option.value = id; return option; })); byId('model-status').textContent = `已获取 ${data.models.length} 个模型`; }
