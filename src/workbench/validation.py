...
DEFAULTS = {
    ...
    'ai_provider': '', 'ai_protocol': 'openai',
...
def validate_options(data, require_query=True):
    ...
    if not isinstance(result['ai_provider'], str) or len(result['ai_provider']) > 80:
        raise ValueError('AI 服务无效')
    result['ai_provider'] = result['ai_provider'].strip().lower() or os.environ.get('DEFAULT_AI_SERVICE', 'openai').strip().lower()
    profile = get_provider(result['ai_provider'])
    if profile is None:
        raise ValueError('不支持的 AI 服务')
    if result['ai_protocol'] not in {'openai', 'openai_responses', 'gemini', 'anthropic'}:
        raise ValueError('接口类型无效')
    if profile.api_type in {'gemini', 'anthropic'} and result['ai_protocol'] != profile.api_type:
        result['ai_protocol'] = profile.api_type
    elif profile.api_type == 'openai' and result['ai_protocol'] not in {'openai', 'openai_responses'}:
        raise ValueError('该服务仅支持 OpenAI 兼容接口')
    if result['ai_protocol'] == 'openai_responses' and not profile.supports_responses:
        raise ValueError(f'{profile.label} 未声明支持 OpenAI Responses，请改用 Chat Completions')
