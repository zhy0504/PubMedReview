...
sealed class WorkbenchTray : Form {
    ...
    Button environmentCheckButton;
    Button environmentRepairButton;
    Button pandocInstallButton;
    Button pandocUpdateButton;
    OwnedProcess owned;
    OwnedProcess environmentProcess;
    bool quitting, ready, probing;
    bool environmentBusy;
    ...
    public WorkbenchTray() {
        ...
        environmentCheckButton = AddButton(actions, "检测环境", CheckEnvironment);
        environmentRepairButton = AddButton(actions, "修复环境/依赖", RepairEnvironment);
        pandocInstallButton = AddButton(actions, "安装 Pandoc", InstallPandoc);
        pandocUpdateButton = AddButton(actions, "更新 Pandoc", UpdatePandoc);
        ...
        menu.Items.Add("检测环境", null, delegate { CheckEnvironment(); });
        menu.Items.Add("修复环境/依赖", null, delegate { RepairEnvironment(); });
        menu.Items.Add("安装 Pandoc", null, delegate { InstallPandoc(); });
        menu.Items.Add("更新 Pandoc", null, delegate { UpdatePandoc(); });
        ...
        FormClosed += delegate { timer.Stop(); tray.Visible = false; tray.Dispose(); if (environmentProcess != null) environmentProcess.Dispose(); if (owned != null) owned.Dispose(); };
        Shown += delegate { StartService(); };
        timer.Interval = 1000; timer.Tick += delegate { CheckService(); };
    }
    Button AddButton(Control parent, string text, Action action) { var button = new Button(); button.Text = text; button.AutoSize = true; button.Click += delegate { action(); }; parent.Controls.Add(button); return button; }
    ...
    void CheckService() {
        ...
        ThreadPool.QueueUserWorkItem(delegate {
            ...
            try { BeginInvoke(new Action(delegate { probing = false; if (ok && !quitting && !owned.Process.HasExited) { ready = true; Log("Web 服务已就绪。"); CheckEnvironment(); OpenWeb(); } })); } catch (InvalidOperationException) {}
        });
    }
    ...
    void CheckEnvironment() { StartEnvironmentCommand("--status"); }
    void RepairEnvironment() { StartEnvironmentCommand("--repair"); }
    void InstallPandoc() { StartEnvironmentCommand("--pandoc"); }
    void UpdatePandoc() { StartEnvironmentCommand("--update-pandoc"); }
    void SetEnvironmentButtons(bool enabled) {
        if (environmentCheckButton != null) environmentCheckButton.Enabled = enabled;
        if (environmentRepairButton != null) environmentRepairButton.Enabled = enabled;
        if (pandocInstallButton != null) pandocInstallButton.Enabled = enabled;
        if (pandocUpdateButton != null) pandocUpdateButton.Enabled = enabled;
    }
    void StartEnvironmentCommand(string arguments) {
        if (environmentBusy) { Log("环境操作正在运行，请等待完成。"); return; }
        environmentBusy = true; SetEnvironmentButtons(false);
        string command = "$python = $null; if (Test-Path './.venv/Scripts/python.exe') { $python = (Resolve-Path './.venv/Scripts/python.exe').Path } elseif (Get-Command python -ErrorAction SilentlyContinue) { $python = (Get-Command python).Source } elseif (Get-Command py -ErrorAction SilentlyContinue) { $python = 'py' } else { throw '未找到 Python，请先安装 Python 3.10 或更高版本。' }; & $python './tools/environment_manager.py' " + arguments + "; exit $LASTEXITCODE";
        try {
            var runner = new OwnedProcess(root, command, delegate(string line) { Log("[ENV] " + line); });
            environmentProcess = runner;
            ThreadPool.QueueUserWorkItem(delegate {
                runner.Process.WaitForExit();
                int exitCode = runner.Process.ExitCode;
                try {
                    BeginInvoke(new Action(delegate {
                        if (environmentProcess == runner) environmentProcess = null;
                        runner.Dispose();
                        environmentBusy = false; SetEnvironmentButtons(true);
                        Log(exitCode == 0 ? "环境操作完成。" : "环境操作失败，退出码：" + exitCode);
                    }));
                } catch (InvalidOperationException) { runner.Dispose(); }
            });
        } catch (Exception error) {
            environmentBusy = false; SetEnvironmentButtons(true); Log("环境操作启动失败：" + error.Message);
        }
    }
    ...
    [STAThread] static int Main(string[] args) {
        if (args.Length == 1 && args[0] == "--self-test") {
            ...
            try {
                ...
                File.WriteAllText(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "logs/tray-self-test.txt"), "PASS: environment check, startup, redirected output, parent and child termination"); return 0;
